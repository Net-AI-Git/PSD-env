...
def main(stop_event=None, config_dict=None):
    ...
    if config.FULL_ENVELOPE:
        ...
        # --- Single output subfolder for this run (date-time) ---
        ...
        for filename in sorted(os.listdir(input_directory)):
            ...
            # Add jobs to the collection; all use the same run output directory
            for job in jobs_from_file:
                all_jobs.append((job, run_output_dir, config_dict, mp_stop_event))
    ...
    logger.info(f"All results have been saved in '{config.OUTPUT_DIR}'.")
    logger.info(f"{'=' * 60}")
